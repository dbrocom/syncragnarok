...
{	//Insert the barebones to then update the rest.
	...
	if( memcmp(&p->bgstats, &cp->bgstats, sizeof(struct s_battleground_stats)) )
	{
		if( SQL_ERROR == Sql_Query(sql_handle, "REPLACE INTO `char_bg` ("
			...
			" VALUES ('%d','%u','%u','%u','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%u','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d')",
			p->char_id,
...
int memitemdata_to_sql(const struct item items[], int max, int id, int tableswitch)
{
		...
		for( i = 0; i < max; ++i )
		{
			...
			{ // Same Item - Checking if this need to be updated.
				...
				{ // Differences founds, updated required
					StringBuf_Clear(&buf);
					StringBuf_Printf(&buf, "UPDATE `%s` SET `amount`='%d', `equip`='%d', `identify`='%d', `refine`='%d',`attribute`='%d', `expire_time`='%u', `serial`='%u', `bound`='%d'",
						tablename, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].serial, items[i].bound);
					for( j = 0; j < MAX_SLOTS; ++j )
	...
	StringBuf_Clear(&buf);
	StringBuf_Printf(&buf, "INSERT INTO `%s`(`%s`, `nameid`, `amount`, `equip`, `identify`, `refine`, `attribute`, `expire_time`, `serial`, `bound`", tablename, selectoption);
	for( j = 0; j < MAX_SLOTS; ++j )
		StringBuf_Printf(&buf, ", `card%d`", j);
	...
	for( i = 0; i < max; ++i )
	{
		...
		StringBuf_Printf(&buf, "('%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u', '%u', '%d'",
			id, items[i].nameid, items[i].amount, items[i].equip, items[i].identify, items[i].refine, items[i].attribute, items[i].expire_time, items[i].serial, items[i].bound);
		for( j = 0; j < MAX_SLOTS; ++j )
...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`save_map`,`save_x`,`save_y`,`partner_id`,`father`,`mother`,`child`,`fame`,`rename`,`delete_date`,`robe`,`playtime`,`elemental_id`"
