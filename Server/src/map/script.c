...
BUILDIN_FUNC(storeitem)
{
	...
	if( result )
	{
		...
		if( result && log_config.enable_logs&LOG_SCRIPT_TRANSACTIONS )
			log_pick_pc(sd, "N", nameid, amount, NULL, it.serial);
	}
...
BUILDIN_FUNC(rentitem)
{
	...
	if( log_config.enable_logs&LOG_SCRIPT_TRANSACTIONS )
		log_pick_pc(sd, "N", nameid, 1, NULL, it.serial);
...
BUILDIN_FUNC(itembound2)
{
	...
	for( i = 0; i < amount; i++ )
	{
		...
		//Logs items, got from (N)PC scripts [Lupus]
		if( log_config.enable_logs&0x40 )
			log_pick_pc(sd, "N", nameid, 1, &item_tmp, item_tmp.serial );
...
static void buildin_delitem_delete(struct map_session_data* sd, int idx, int* amount, bool delete_items)
{
	...
	if( delete_items )
	{
		...
		if( log_config.enable_logs&0x40 )
		{
			log_pick_pc(sd, "N", inv->nameid, -delamount, inv, inv->serial);
		}
		//Logs
...
BUILDIN_FUNC(successenchant)
{
	...
	if( log_config.enable_logs&0x40 )
		log_pick_pc(sd, "N", sd->status.inventory[i].nameid, -1, &sd->status.inventory[i], sd->status.inventory[i].serial );
	...
	if( battle_config.channel_announces&0x8 )
	{
		sprintf(chat_announce, msg_txt(892), vending_chat_nick, sd->status.name, sd->inventory_data[i]->jname, sd->inventory_data[i]->slot, itemdb_search(enchant)->jname);
...
BUILDIN_FUNC(successrefitem)
{
	...
	if(i >= 0) {
		...
		if( battle_config.channel_announces&0x4 && sd->inventory_data[i]->wlv >= 0 && sd->inventory_data[i]->wlv <= 4 && sd->status.inventory[i].refine >= announce_refine[sd->inventory_data[i]->wlv] )
		{ // Announces Refines to Chat
			char chat_announce[256];
			sprintf(chat_announce, msg_txt(821), vending_chat_nick, sd->status.name, sd->status.inventory[i].refine, sd->inventory_data[i]->jname, sd->inventory_data[i]->slot);
			clif_channel_message(server_channel[CHN_VENDING], chat_announce, 26);
		}
...
BUILDIN_FUNC(failedrefitem)
{
	...
	if(i >= 0) {
		...
		{ // Announces Refines to Chat
			char chat_announce[256];
			sprintf(chat_announce, msg_txt(822), vending_chat_nick, sd->status.name, sd->status.inventory[i].refine, sd->inventory_data[i]->jname, sd->inventory_data[i]->slot);
...
BUILDIN_FUNC(failedrefitemR) // by jakeRed
{
	...
	if( i >= 0 )
	{
		...
		{ // Announces Refines to Chat
			char chat_announce[256];
			sprintf(chat_announce, msg_txt(904), vending_chat_nick, sd->status.name, sd->status.inventory[i].refine, sd->inventory_data[i]->jname, sd->inventory_data[i]->slot,(sd->status.inventory[i].refine - ref));
			clif_channel_message(server_channel[CHN_VENDING], chat_announce, 1);
		}
...
BUILDIN_FUNC(successremovecards)
{
	...
	{	// カードを取り除いたアイテム所得
		...
		//Logs items, got from (N)PC scripts [Lupus]
		if(log_config.enable_logs&0x40)
			log_pick_pc(sd, "N", item_tmp.nameid, 1, &item_tmp, item_tmp.serial );
	...
	for( c = sd->inventory_data[i]->slot - 1; c >= 0; --c )
	{
		if( sd->status.inventory[i].card[c] && itemdb_type(sd->status.inventory[i].card[c]) == IT_CARD )
		{
			...
			{// add cards to inventory, clear
				...
				//Logs items, got from (N)PC scripts [Lupus]
				if(log_config.enable_logs&0x40)
					log_pick_pc(sd, "N", item_tmp.nameid, 1, NULL, 0);
...
BUILDIN_FUNC(clearitem)
{
	...
	for (i=0; i<MAX_INVENTORY; i++) {
		if (sd->status.inventory[i].amount) {
			...
			//Logs items, got from (N)PC scripts [Lupus]
			if(log_config.enable_logs&0x40)
				log_pick_pc(sd, "N", sd->status.inventory[i].nameid, -sd->status.inventory[i].amount, &sd->status.inventory[i], sd->status.inventory[i].serial );
...
struct script_function buildin_func[] = {
	...
	// Rol NPC Script [Oriol]
	BUILDIN_DEF(pcthink,"is"),
	BUILDIN_DEF(playertalk,"is"),
