...
bool channel_create(struct map_session_data *sd, const char* name, const char* pass, short type, short color)
{
	...
	{
		clif_displaymessage(sd->fd,  msg_txt(802));
...
void channel_join(struct map_session_data *sd, const char* name, const char* pass, bool invite)
{
	...
	if( !invite && cd->pass[0] && strcmp(cd->pass, pass) != 0 )
	{ // Check password only if not invited
		clif_displaymessage(sd->fd, msg_txt(808));
		return;
	}

	if( !invite && cd->type == CHN_GAMEMASTER && pc_isGM(sd) < battle_config.lowest_gm_level )
	{
...
void channel_message(struct map_session_data *sd, const char* channel, const char* message)
{
	...
	{ // Channel commands
		...
		if( !strncasecmp(message, ".item ", 6) && len > 0 && cd->type == CHN_VENDING && vendingbot_timer < gettick() )
		{
			...
			for( i = 0; i < count; i++ )
			{
				...
				{
					sprintf(output, msg_txt(829), vending_chat_nick, item_array[i]->jname, MinPrice[i], b_sd[i]->status.name, map[b_sd[i]->bl.m].name, b_sd[i]->bl.x, b_sd[i]->bl.y, MaxPrice[i], total[i], amount[i]);
					clif_channel_message(cd, output, -1);
				}
			}
...
void do_init_channel(void)
{
	...
	// Basic Server Channels
	channel_create(NULL, main_chat_nick, "", CHN_MAIN, 38);
	channel_create(NULL, vending_chat_nick, "", CHN_VENDING, 38);
	channel_create(NULL, battleground_chat_nick, "", CHN_BATTLEGROUND, 38);
	channel_create(NULL, gamemaster_chat_nick, "", CHN_GAMEMASTER, 38);
}
