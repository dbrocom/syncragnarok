...
int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points)
{
	...
	{ // Special Shop
		...
		{ // BG Shop
			...
			for( j = 0; j < 3; j++ )
				pc_delitem(sd,indexs[j],price,0,0);
		}
		else
		{
...
int npc_selllist(struct map_session_data* sd, int n, unsigned short* item_list)
{
	...
	for( i = 0; i < n; i++ )
	{
		...
		if( log_config.enable_logs&0x20 )
			log_pick_pc(sd, "S", nameid, -amount, &sd->status.inventory[idx], sd->status.inventory[idx].serial);
		//Logs
...
void npc_parsesrcfile(const char* filepath)
{
	...
	for( p = skip_space(buffer); p && *p ; p = skip_space(p) )
	{
		...
		else if( (!strcasecmp(w2,"shop") || !strcasecmp(w2,"cashshop") || !strcasecmp(w2,"sshop")) && count > 3 )
		{
