...
void buyingstore_create(struct map_session_data* sd, int zenylimit, unsigned char result, const char* storename, const uint8* itemlist, unsigned int count)
{
	...
	if( map[sd->bl.m].flag.novending || map_getcell(sd->bl.m, sd->bl.x, sd->bl.y, CELL_CHKNOBOARDS) )
	{// custom: no vending maps/cells
		clif_displaymessage(sd->fd, msg_txt(276)); // "You can't open a shop on this map"
		return;
	}
...
void buyingstore_trade(struct map_session_data* sd, int account_id, unsigned int buyer_id, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		{
			log_pick_pc(sd, "B", nameid, -((int)amount), &sd->status.inventory[index], sd->status.inventory[index].serial);
			log_pick_pc(pl_sd, "B", nameid, amount, &sd->status.inventory[index], sd->status.inventory[index].serial);
		}
		if( log_config.zeny )
