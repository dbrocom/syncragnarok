...
char vending_chat_nick[16]		= "#vending";
char battleground_chat_nick[16]	= "#bg";
char gamemaster_chat_nick[16]	= "#gm";
...
struct region_data region[MAX_REGIONS];
...
void map_reqnickdb(struct map_session_data * sd, int charid)
{
	...
	if( battle_config.hunting_reserved_char_id && battle_config.hunting_reserved_char_id == charid )
	{
		clif_solved_charname(sd->fd, charid, "Hunting");
		return;
	}
...
int inter_config_read(char *cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(strcmpi(w1, "vending_chat_nick")==0)
			safestrncpy(vending_chat_nick, w2, sizeof(vending_chat_nick));
		else
		if(strcmpi(w1, "battleground_chat_nick")==0)
			safestrncpy(battleground_chat_nick, w2, sizeof(battleground_chat_nick));
		else
		if(strcmpi(w1, "gamemaster_chat_nick")==0)
			safestrncpy(gamemaster_chat_nick, w2, sizeof(gamemaster_chat_nick));
