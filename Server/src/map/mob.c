...
#define RUDE_ATTACKED_COUNT 3	//After how many rude-attacks should the skill be used?
...
int mob_once_spawn_especial(struct map_session_data *sd, const char *mapname, short x, short y, const char *mobname, int class_, int amount, const char *event, int hp_mod, short size, short ai_type, bool no_slaves, short allow_warp, short hp_show, bool announce_hprate, bool announce_killer, bool no_expdrop, int TeamID, short item_drop, short item_amount, bool is_war, short exp_boost, bool drop_boost)
{
	...
	for( count = 0; count < amount; count++ )
	{
		...
		if( map[m].flag.pvp ) md->option.party_id = TeamID;
...
static bool mob_ai_sub_hard(struct mob_data *md, unsigned int tick)
{
	...
	{	//Loot time.
		...
		if(log_config.enable_logs&0x10)	//Logs items, taken by (L)ooter Mobs [Lupus]
			log_pick_mob(md, "L", fitem->item_data.nameid, fitem->item_data.amount, &fitem->item_data, fitem->item_data.serial);
...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, bool flag)
{
	...
	{	//Logs items, dropped by mobs [Lupus]
		if( loot )
			log_pick_mob(md, "L", ditem->item_data.nameid, -ditem->item_data.amount, &ditem->item_data, ditem->item_data.serial);
		else
			log_pick_mob(md, "M", ditem->item_data.nameid, -ditem->item_data.amount, NULL, ditem->item_data.serial);
	}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	if( src && src->type == BL_PC )
	{
		...
		if( !status->mode&MD_BOSS )
			bonusdrop = pc_region_bonus(sd, 3);
	}
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP; i++)
		{
			...
			if( mvp_sd && battle_config.channel_announces&0x2 && i_data->type == IT_CARD )
			{ // Card Drop Announce on Vending Channel
				char message[128];
				sprintf(message, msg_txt(823), vending_chat_nick, mvp_sd->status.name, i_data->jname);
