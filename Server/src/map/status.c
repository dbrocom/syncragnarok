...
//		return 0; //Cannot damage a bl not on a map, except when "charging" hp/sp
	...
	if( hp && !(flag&(1|8)) ) {
		if( sc ) {
			...
			if( src && !(flag&64) && skill != PA_PRESSURE )
...
int status_base_amotion_pc(struct map_session_data* sd, struct status_data* status)
{
	int amotion;
	if( battle_config.renewal_system_enable )
	{ // baseaspd + floor(sqrt((agi^2/2) + (dex^2/5))/4 + (potskillbonus*agi/200) + shieldpenalty)
		int i, shield = 0;
		...
		if( (i = sd->equip_index[EQI_HAND_L]) >= 0 && sd->inventory_data[i] && sd->inventory_data[i]->type == IT_ARMOR )
			shield = shield_penalty[pc_class2idx(sd->status.class_)];

		amotion -= (int)(floor(sqrt((pow(status->agi,2)/2.) + (pow(status->dex,2.)/5.))/4. - shield) * 10.);
	}
	else
	{
		// base weapon delay
...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	for(i=0;i<EQI_MAX-1;i++) {
		...
		if( battle_config.renewal_system_enable )
			status->matk_min += sd->inventory_data[index]->matk; // Added for Renewal
		...
		if(sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if( battle_config.renewal_system_enable )
			{
				...
				status->matk_max = (wlv * sd->inventory_data[index]->matk) / 10; // To hold Min~Max matk diff
			}
...
// Unlike other stats, ASPD rate modifiers from skills/SCs/items/etc are first all added together, then the final modifier is applied
	...
	if( battle_config.renewal_system_enable )
	{
		if( sd->class_&JOBL_THIRD )
			status->amotion = cap_value(i,70,2000); // 193 Max ASPD
		else
			status->amotion = cap_value(i,100,2000); // 190 Max ASPD
	}
	else
...
// if(flag&SCB_RANGE)
	...
	if( flag&SCB_MATK )
	{
		if( battle_config.renewal_system_enable )
		{
			...
			status->matk = status_calc_matk(bl,sc,status->matk);
	...
	if(flag&SCB_ASPD) {
		...
		if( bl->type&BL_PC )
		{
			...
			if( battle_config.renewal_system_enable )
			{
				int limit = (sd->class_&JOBL_THIRD) ? 70 : 100;
...
static unsigned short status_calc_watk(struct block_list *bl, struct status_change *sc, int watk)
{
	...
	if(sc->data[SC_SHIELDSPELL_DEF] && sc->data[SC_SHIELDSPELL_DEF]->val1 == 1)
...
int status_get_guild_id(struct block_list *bl)
{
	...
	case BL_MOB:
	{
		...
		if (index && md->option.is_war && !map_flag_vs(bl->m))
			return md->option.guild_id;
	}
		break;
	case BL_HOM:
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_CASHSHOP:
		if( sd && sd->state.cashshop && sd->vender_id )
		{
	...
	case SC_ELECTRICSHOCKER:
		if( --(sce->val4) >= 0 )
		{
			...
			clif_skill_damage(bl,bl,tick,0,0,-30000,1,RA_ELECTRICSHOCKER,sce->val1,5);
			sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
			return 0;
		}
		break;
...
{// Job-specific values (weight, HP, SP, ASPD)
	...
	if( battle_config.renewal_system_enable )
	{
		shield_penalty[idx] = atoi(fields[5]);
		c++;
	}

	for(i = 0; i < MAX_WEAPON_TYPE; i++)
	{
		aspd_base[idx][i] = atoi(fields[i+c]);
	}
	return true;
}
