...
// Regional Bonus
int pc_region_bonus(struct map_session_data *sd, int bonus)
{
	int m, index;
	if( sd == NULL )
		return 0;
	...
	if( region == 0 || sd->status.guild_id == 0 || sd->status.guild_id != region[index].guild_id )
		return 0;

	switch( bonus )
	{
		case 0: return region[index].bexp; break; // Base Exp
		case 1: return region[index].jexp; break; // Job Exp
		case 2: return region[index].vending_tax; break; // Vending Tax
		case 3: return region[index].bdrop; break; // Drop Bonus
	}

	return 0;
}

// Regional control for Guild
int pc_region(struct map_session_data *sd, int index)
{
	if( sd == NULL || sd->status.guild_id == 0 || map_flag_vs(sd->bl.m) )
		return 0;
	
	if( region[index].guild_id == sd->status.guild_id )
		return 1;
		...
		for( sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); sd = (TBL_PC*)mapit_next(iter) )
		{
			...
			{ // Inventory Removal
				if( sd->status.inventory[index].nameid != nameid )
					continue;
				pc_delitem(sd, index, sd->status.inventory[index].amount, 0, 0);
			}
			...
			{ // Cart Removal
				if( sd->status.cart[index].nameid != nameid )
					continue;
				pc_cart_delitem(sd, index, sd->status.cart[index].amount, 0);
			}
...
int pc_isequip(struct map_session_data *sd,int n)
{
	...
	if(map[sd->bl.m].flag.pvp && ((item->flag.no_equip&1) || !pc_isAllowedCardOn(sd,item->slot,n,1)))
		return 0;
	if(map_flag_gvg2(sd->bl.m) && ((item->flag.no_equip&2) || !pc_isAllowedCardOn(sd,item->slot,n,2)))
		return 0;
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	if( battle_config.channel_system_enable )
	{
		...
		if( sd->channels&1 )
			channel_join(sd, main_chat_nick, "", false);
		if( sd->channels&2 )
			channel_join(sd, vending_chat_nick, "", false);
		if( sd->channels&4 )
			channel_join(sd, battleground_chat_nick, "", false);
		if( pc_isGM(sd) >= battle_config.lowest_gm_level && (sd->channels&8) )
			channel_join(sd, gamemaster_chat_nick, "", false);
	}
...
/*==========================================
 * ƒAƒCƒeƒ€‚ð”ƒ‚Á‚½ŽbÉAV‚µ‚¢ƒAƒCƒeƒ€—“‚ðŽg‚¤‚©A
 * 3–œŒÂ§ŒÀ‚É‚©‚©‚é‚©Šm”F
 *------------------------------------------*/
...
/*==========================================
 * ƒAƒCƒeƒ€‚ð’T‚µ‚ÄAƒCƒ“ƒfƒbƒNƒX‚ð•Ô‚·
 *------------------------------------------*/
...
int pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	if(log_config.enable_logs&0x8)
		log_pick_pc(sd, "P", sd->status.inventory[n].nameid, -amount, (struct item*)&sd->status.inventory[n], sd->status.inventory[n].serial);
	//Logs
...
int pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	if (
		...
		(map_flag_gvg2(sd->bl.m) && item->flag.no_equip&2) || // GVG
		((map[sd->bl.m].flag.battleground || map[sd->bl.m].flag.pvp_event) && item->flag.no_equip&3) || // BG - PVP Event
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	if( !map[sd->bl.m].flag.ancient && sd->status.inventory[n].card[0] == CARD0_CREATE && char_id && (char_id == battle_config.hunting_reserved_char_id || char_id == battle_config.bg_reserved_char_id || char_id == battle_config.woe_reserved_char_id || pc_famerank(char_id, MAPID_ALCHEMIST)) )
	{
...
int pc_steal_item(struct map_session_data *sd,struct block_list *bl, int lv)
{
	...
	if(log_config.enable_logs&0x80) {
		log_pick_mob(md, "M", itemid, -1, NULL, tmp_item.serial);
		log_pick_pc(sd, "P", itemid, 1, NULL, tmp_item.serial);
	}
...
int pc_setpos(struct map_session_data* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	sd->state.changeregion = (map[sd->bl.m].region != map[m].region);
	sd->mapindex = mapindex;
	sd->bl.m = m;
...
static void pc_calcexp(struct map_session_data *sd, unsigned int *base_exp, unsigned int *job_exp, struct block_list *src)
{
	...
	ebonus += pc_region_bonus(sd, 0);
	jbonus += pc_region_bonus(sd, 1);
	...
	for( i = 0; i < MAX_INVENTORY; i++)
	{
		...
		{ // check for forbiden items.
			int flag =
					...
					| (map_flag_gvg2(sd->bl.m)?2:0)
					| ((map[sd->bl.m].flag.battleground || map[sd->bl.m].flag.pvp_event)?3:0);
...
/*==========================================
 * sd‚ªdstsd‚ÆŒ‹¥(dstsd¨sd‚ÌŒ‹¥?—‚à“¯ŽbÉs‚¤)
 *------------------------------------------*/
