...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv,int element)
{
	...
	if( sc && sc->count )
	{
		...
		if( sd && (sce = sc->data[SC_HAGALAZ]) && flag&BF_WEAPON && damage > 0 )
		{
			sce->val2 -= damage;
	...
	{	//There is a total damage value
		...
		if( wd.damage2 )
		{
			...
			if( map_flag_gvg2(target->m) )
				...
				wd.damage = battle_calc_pvpevent_damage(src,target,wd.damage2,wd.div_,skill_num,skill_lv,wd.flag);
		}
	}
...
#define BON_ADD2( a , b ) { damage_bonus += a; if( flag.lh ) damage_bonus_ += b; }
	...
	if( flag.hit && !flag.infdef )
	{
		...
		// -------------------------------------------------
		if( (skillmod = skillmod + c_racemod[2]) < 0 )
			skillmod = 0; // To avoid Negative Damage
...
#define BON_RATE2( a , b ) { damage_bonus = damage_bonus * (a) / 100; if( flag.lh ) damage_bonus_ = damage_bonus_ * (b) / 100; }
	...
	if( flag.hit && !flag.infdef )
	{ // Reduction of Target Side Cards except Race.
...
struct Damage battle_calc_magic_attack_renewal(struct block_list *src, struct block_list *target, int skill_id, int skill_lv, int mflag)
{
	...
	if( !flag.infdef )
	{
		...
		default:
			...
			{ // Matk comes from Rune Knight, into the SC : va2 = bonus from skill val3 = min matk val4 = max matk.
				if( sc && sc->data[SC_ENCHANTBLADE] )
					ad.damage += sc->data[SC_ENCHANTBLADE]->val3 + rand()%(1 + sc->data[SC_ENCHANTBLADE]->val4 - sc->data[SC_ENCHANTBLADE]->val3) + sc->data[SC_ENCHANTBLADE]->val2;
				else
					return ad; // Should not happen...
			}
			else
			{
				ad.damage = (sstatus->matk * ((sd && sd->matk_rate != 100) ? sd->matk_rate : 100) / 100) + sstatus->matk_min;
				if( sc && sc->data[SC_RECOGNIZEDSPELL] )
				{ // mATK Max
					if( sd )
			...
			if( sc && skill_id == RK_ENCHANTBLADE && sc->data[SC_ENCHANTBLADE] )
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, unsigned int tick, int flag)
{
	...
	{ // Autocast one Bolt depending on status change.
		int sid = 0;
		if( sc->data[SC_TROPIC_OPTION] ) sid = sc->data[SC_TROPIC_OPTION]->val3;
		else if( sc->data[SC_CHILLY_AIR_OPTION] ) sid = sc->data[SC_CHILLY_AIR_OPTION]->val3;
		else if( sc->data[SC_WILD_STORM_OPTION] ) sid = sc->data[SC_WILD_STORM_OPTION]->val2;
		else if( sc->data[SC_UPHEAVAL_OPTION] ) sid = sc->data[SC_UPHEAVAL_OPTION]->val2;

		if( sid )
			skill_castend_damage_id(src, target, sid, pc_checkskill(sd,sid), tick, flag);
	}
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	{	//Checks on source master
		...
		case BL_MOB:
		{
			...
			if( md->option.is_event )
			{
				if( t_bl->type == BL_PC )
				{
					short index = map[m].region;
					struct map_session_data *sd = BL_CAST(BL_PC, t_bl);
					if( index && md->option.is_war && !map_flag_vs(m) && !guild_isenemy(md->option.guild_id, sd->status.guild_id) )
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			else if( !map[m].flag.battleground && s_guild && t_guild && (s_guild == t_guild || (!map[m].flag.gvg_noalliance && guild_isallied(s_guild, t_guild))) && !(map[m].flag.pvp && map[m].flag.pvp_noguild) && !map_pvpevent(m) )
				state |= BCT_GUILD;
...
// Ancient WoE
	...
	{ "hunting_reserved_char_id",           &battle_config.hunting_reserved_char_id,        999998, 0,      INT_MAX,        },
