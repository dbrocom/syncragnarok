...
int should_log_item(int filter, int nameid, int amount)
{
	...
	if ((filter&1) || // Filter = 1, we log any item
		(item_data->log) || // Should have log because of item_db_log
...
//Mob picked item
int log_pick_mob(struct mob_data *md, const char *type, int nameid, int amount, struct item *itm, uint64 serial)
{
	char* mapname;
	...
	{
		if( itm == NULL ) { //We log common item
			if (SQL_ERROR == Sql_Query(logmysql_handle, "INSERT DELAYED INTO `%s` (`time`, `char_id`, `type`, `nameid`, `amount`, `map`, `serial`) VALUES (NOW(), '%d', '%s', '%d', '%d', '%s', '%u')",
				log_config.log_pick_db, md->class_, type, nameid, amount, mapname, serial) )
			{
				Sql_ShowDebug(logmysql_handle);
				return 0;
			}
		} else { //We log Extended item
			if (SQL_ERROR == Sql_Query(logmysql_handle, "INSERT DELAYED INTO `%s` (`time`, `char_id`, `type`, `nameid`, `amount`, `refine`, `card0`, `card1`, `card2`, `card3`, `map`, `serial`) VALUES (NOW(), '%d', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%s', '%u')",
				log_config.log_pick_db, md->class_, type, itm->nameid, amount, itm->refine, itm->card[0], itm->card[1], itm->card[2], itm->card[3], mapname, serial) )
			{
				Sql_ShowDebug(logmysql_handle);
				return 0;
			}
