...
#define map_flag_vs(m) (map[m].flag.pvp || map[m].flag.gvg_dungeon || map[m].flag.gvg || (agit_flag && map[m].flag.gvg_castle) || map[m].flag.battleground || (pvpevent_flag && map[m].flag.pvp_event))
...
// PK Mode - Not PK maps
#define map_flag_nopvpmode(m) (map[m].flag.gvg || map[m].flag.pvp || map[m].flag.nopvpmode || map[m].flag.gvg_castle || map[m].flag.battleground || (pvpevent_flag && map[m].flag.pvp_event))
// No Guild War Maps
#define map_flag_noguildwar(m) (map[m].flag.town || map[m].flag.gvg || map[m].flag.pvp || map[m].flag.noguildwar || map[m].flag.gvg_castle || map[m].flag.battleground || (pvpevent_flag && map[m].flag.pvp_event))
// No Kill Steal Protection
#define map_flag_ks(m) (map[m].flag.town || map[m].flag.pvp || map[m].flag.gvg || map[m].flag.battleground || (pvpevent_flag && map[m].flag.pvp_event))
// PvP Event Map
...
enum _sp {
	...
	SP_ADD_HEAL_RATE,SP_ADD_HEAL2_RATE,SP_FIXCASTRATE, //1090-1092
...
struct map_data {
	...
	int m;
	unsigned short region; // Regional Data [Zephyrus]
...
extern struct region_data region[];
...
extern char vending_chat_nick[16];
extern char battleground_chat_nick[16];
extern char gamemaster_chat_nick[16];
